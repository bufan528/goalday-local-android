...
Current app status after the calendar-import pass: local/offline planner with three root tabs, topic-style book covers, template center, topic detail surface, save-as-book flow, standalone target-detail-style book pages, handbook page turn, task pool, schedule todo/done, schedule time/repeat fields, Android system calendar import, rich diary editor, and backup settings.

## Overall Gap

Estimated completion against the reference APK's useful offline planner surface: **about 55-60%**.
...
1. **Diary media blocks**: add image block support and date picker to diary pages.
2. **Long image export**: export handbook/diary pages as an image file.
3. **Widgets**: local schedule widget and quick diary add widget.
...
The largest remaining visible mismatch is now **diary media blocks + long image export**. System calendar import is available, but diary pages still lack reference-style image blocks and export/print behavior.
