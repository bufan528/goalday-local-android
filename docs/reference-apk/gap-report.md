...
Current app status after the diary-date pass: local/offline planner with three root tabs, topic-style book covers, template center, topic detail surface, save-as-book flow, standalone target-detail-style book pages, handbook page turn, task pool, schedule todo/done, schedule time/repeat fields, Android system calendar import, structured diary editor with image blocks, target-linked diary chips, explicit diary date picker, diary and handbook schedule long-image export/share, small/large schedule widgets, quick diary widget, immediate schedule widget refresh hooks, and backup settings.

## Overall Gap

Estimated completion against the reference APK's useful offline planner surface: **about 90-92%**.
...
1. **Calendar import polish**: add selectable calendars, import preview, and broader range import if needed.
2. **Schedule polish**: add a richer repeat editor and per-occurrence edit behavior if needed.
...
The largest remaining visible mismatch is now **import/schedule polish**. Core local schedule, diary, import, export/share, widget surfaces, diary target links, and diary date controls exist, but the app still lacks selectable calendar import and a richer repeat editor.
