...
Current app status after the target-detail pass: local/offline planner with three root tabs, topic-style book covers, template center, topic detail surface, save-as-book flow, standalone target-detail-style book pages, handbook page turn, task pool, schedule todo/done, rich diary editor, and backup settings.

## Overall Gap

Estimated completion against the reference APK's useful offline planner surface: **about 45-50%**.
...
1. **Schedule repeat/time model**: add time slots and repeat options to local schedules.
2. **Calendar import**: implement optional Android system calendar import into the local schedule repository.
3. **Diary media blocks**: add image block support and date picker to diary pages.
4. **Long image export**: export handbook/diary pages as an image file.
5. **Widgets**: local schedule widget and quick diary add widget.
6. **Target detail polish**: add richer note/deadline fields if the local workflow needs them.

## Immediate Next Fix

The largest remaining visible mismatch is now **schedule repeat/time behavior**. Target pages no longer use a plain checklist surface, but local schedules still lack the reference APK's repeat picker, time fields, and move-target flow.
